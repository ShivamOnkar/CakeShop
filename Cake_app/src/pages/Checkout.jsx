...
const Checkout = () => {
  ...
  const fetchUserAddresses = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showNotification(result.error || "Failed to load addresses", "error");
      }
    } catch (error) {
  ...
  // Handle adding new address using AuthContext
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem(
          "latestOrder",
          JSON.stringify({
            orderId: responseData.order?._id || responseData._id,
            total: responseData.order?.totalAmount || orderData.totalAmount,
            address: {
              name:
                responseData.order?.customerName ||
                orderData.customerName ||
                "Customer",
              street:
                responseData.order?.shippingAddress?.street ||
                orderData.shippingAddress?.street ||
                "",
              city:
                responseData.order?.shippingAddress?.city ||
                orderData.shippingAddress?.city ||
                "",
              state:
                responseData.order?.shippingAddress?.state ||
                orderData.shippingAddress?.state ||
                "",
              zipCode:
                responseData.order?.shippingAddress?.zipCode ||
                orderData.shippingAddress?.zipCode ||
                "",
              country:
                responseData.order?.shippingAddress?.country ||
                orderData.shippingAddress?.country ||
                "",
              phone:
                responseData.order?.customerPhone ||
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Header */}
      <div className="font-bold text-3xl text-red-700 ml-8 mt-6">
        Nandini Cakes
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-center gap-10 mt-8 px-4">
        ...
        <div className="w-full max-w-2xl">
          ...
          {cart.map((item) => (
            <div
              key={item.id}
              className="bg-white border rounded-lg shadow-sm mb-6"
            >
              <div className="flex items-start p-4 gap-3">
                ...
                <div className="flex-1 pr-2">
                  ...
                  <div className="text-sm text-gray-600 mt-1">
                    Please ensure order details are correct — replacement will
                    not be provided
                    <br />
              ...
              <div className="flex justify-between border-t px-4 py-2 text-sm text-gray-600">
                <span>
                  100% eggless, hygienically prepared, and delivered with care.
                </span>
        ...
        <div className="w-full max-w-md">
          <div className="text-lg font-medium mb-3">
            Price Details ({cart.reduce((sum, item) => sum + item.quantity, 0)}{" "}
            Items)
          </div>
            ...
            <div className="flex gap-6 p-6 mb-8">
              <div>
                ...
                <div className="flex items-center border rounded px-2">
                  <button
                    onClick={() =>
                      handleUpdateItem(editingItem.id, editingItem.quantity - 1)
                    }
          ...
          <div className="fixed top-0 right-0 w-full sm:w-1/3 max-w-sm h-full bg-white shadow-lg z-50 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {userAddresses.length > 0 ? (
                userAddresses.map((address) => (
                  <div
                    key={address._id}
                    className="border rounded-lg p-4 relative cursor-pointer hover:border-red-600 transition-colors"
            ...
            <div className="p-4 border-t">
              <button
                ...
                className="w-full border border-red-600 text-red-600 py-2 rounded-lg mb-3 hover:bg-red-50 transition"
              >
                + Add New Address
              </button>
              <button
                onClick={() =>
                  selectedAddress && handleDeliver(selectedAddress)
                }
          ...
          <div className="fixed top-0 right-0 w-full sm:w-1/3 max-w-sm h-full bg-white shadow-lg z-50 flex flex-col">
            <div className="flex items-center p-4 border-b">
              <button
                ...
                className="text-xl text-gray-600 mr-2 hover:text-gray-800"
              >
                ←
              </button>
              <h2 className="font-semibold text-gray-800">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              <input
                ...
                name="name"
                placeholder="Name *"
                className="w-full border-b py-2 outline-none focus:border-red-600"
